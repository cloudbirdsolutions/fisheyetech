...
export default function ForceClose() {
  ...
  const [action, setAction] = React.useState("forcecomplete");

  const router = useRouter();
  const dispatch = useDispatch<AppDispatch>();
  const auth = useAuth();
  const [endPoint, setEndPoint] = React.useState<string>(
    "/sheetdocid/forcecomplete"
  );
  const {
    data: forceCompleteData,
    isLoading,
    error,
    fetchData: fetchForcecomplete,
  } = useApi<ForceCloseSheet>(endPoint, { method: "GET" });
      ...
      {auth ? (
        <>
          ...
          <ForceTable
            ...
            setRow={setRow}
            setAction={setAction}
            sheetList={forceCompleteData}
            fetchSheets={fetchForcecomplete}
          />
          <ForceLists
            open={open}
            setOpen={setOpen}
