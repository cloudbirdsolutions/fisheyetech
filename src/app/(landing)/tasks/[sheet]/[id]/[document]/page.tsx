...
async function getSheetFields(sheetid: string) {
  try {
    const response = await fetch(
      `http://51.79.147.139:3000/forms/get?id=${sheetid}`,
      {
        method: "GET",
        headers: {
...
async function getDocumentShift(documentId: string) {
  try {
    const response = await fetch(
      `http://51.79.147.139:3000/docshiftstate/get?id=${documentId}`,
      {
        method: "GET",
        headers: {
          Accept: "application/json",
...
async function getDocumentRecords(documentId: string, shiftId: number) {
  try {
    const response = await fetch(
      `http://51.79.147.139:3000/recordmaster/get-records?docId=${documentId}&shiftId=${shiftId}`,
      {
        method: "GET",
        headers: {
          Accept: "application/json",
...
async function getUserPermission(sheetId: string, userId: number) {
  try {
    const response = await fetch(
      `http://51.79.147.139:3000/joballocation/get-permissions?userId=${userId}&sheetId=${sheetId}`,
      {
        method: "GET",
        headers: {
          Accept: "application/json",
...
export default function Log() {
    ...
    parameterMaster: [
      {
        id: "",
        parameterName: "",
        fieldMaster: [{ id: "", fieldName: "", fieldId: "" }],
      },
    ],
  });
  ...
  const [fieldRecord, setFieldRecord] = useState(
    jmespath.search(
      parameters,
      "parameterMaster[].fieldMaster[].{fieldId:id,parameterId:parameterId}"
    )
  );
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      let permissionData = await getUserPermission(
        params.id,
        logintype.data.id
      );
    ...
    const fetchData = async () => {
      ...
      let mergedFiledDocumentRecord = fieldRecord.map((f: any) => {
        ...
        let matchedRecord = documentRecordResp.data.find(
          (rec: Reccod) =>
            rec.fieldId === parseInt(f.fieldId) &&
            rec.parameterId === parseInt(f.parameterId)
        );
  ...
  const getFieldValue = (fieldId: string, parameterId: string) => {
    const matchedRecord = documentRecord.find(
      (rec) =>
        rec.fieldId === parseInt(fieldId) &&
        rec.parameterId === parseInt(parameterId)
    );
  ...
  const updateValue = (
    ...
    let newDocumentRecord = documentRecord.map((rec) =>
      rec.fieldId === parseInt(fieldId) &&
      rec.parameterId === parseInt(paramerterId)
        ? Object.assign({}, rec, { fieldValue: e.target.value })
        : rec
    );
        ...
        >
          ...
          <TabPanel value={index} sx={{ height: 540, overflow: "auto" }}>
            {parameters.parameterMaster && (
              <AccordionGroup size="sm" sx={{ minWidth: "60dvw" }}>
                {parameters.parameterMaster.map((paramter, index) => (
                  <Accordion
                    key={`paramater_${paramter.id}`}
                    ...
                    <AccordionDetails>
                      <Sheet sx={{ height: 400, overflow: "auto" }}>
                        <Table stickyFooter hoverRow>
                          <tbody>
                            {paramter.fieldMaster &&
                              paramter.fieldMaster.map((field) => (
                                <tr key={`field_${field.fieldId}`}>
                                  <td>{field.fieldName}</td>
                                  <td>
                                    <Input
                                      key={`input_key_${field.id}`}
                                      size="sm"
                                      value={getFieldValue(
                                        field.id,
                                        paramter.id
                                      ...
                                      onChange={(e) =>
                                        updateValue(e, field.id, paramter.id)
                                      }
                                      disabled={
                                        getMatchedFieldRecord(
                                          field.id,
                                          paramter.id
                                        )?.transitionId != sheetPermissionId
                                      }
                                    />
                                  </td>
                                </tr>
